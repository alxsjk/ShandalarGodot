...
## Isolated Homelands entry point to the existing Deck Lab, including its
...
func _pack_main() -> void:
	...
	if args.has("--version") or args.has("-V"):
		print("pack_4_deck_lab.gd ", ProjectSettings.get_setting("application/config/version", "unknown"))
		quit(0)
		return
	if args.has("--help") or args.has("-h"):
		print("Pack 4 Deck Lab: isolated Homelands enabled in memory; always --no-elo --procs 1.")
		print(HELP)
		quit(0)
		return
	if not OS.has_feature("shandalar_test"):
		printerr("Use GODOT_EDITOR_CUSTOM_FEATURES=shandalar_test for the Pack 4 Deck Lab")
		quit(2)
		return
	...
	if not CardRegistry.has_card("Giant Oyster"):
		printerr("Pack 4 is missing; set SHANDALAR_PACK_4 to your locally built ZIP")
		quit(2)
		return
	if args.has("--procs"):
		var at := args.find("--procs")
		if at + 1 >= args.size() or args[at + 1] != "1":
			printerr("Pack 4 Deck Lab uses --procs 1; use --jobs for parallel threads")
			quit(2)
			return
