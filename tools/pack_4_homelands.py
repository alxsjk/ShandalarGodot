#!/usr/bin/env python3
"""Build, fetch, or verify the separate Homelands gameplay pack.
...
SOURCE = ROOT / 'packaging/card_packs/pack_4_homelands'
PREFIX = 'card_packs/pack_4_homelands/'
FILE_NAME = 'Pack-4-Homelands.zip'
DEFAULT_OUT = ROOT.parent / 'shandalar-packs' / FILE_NAME
DEFAULT_ART = ROOT.parent / 'shandalar-packs/cache/pack_4_art'
TOOL = 'pack_4_homelands.py'
WORDMARK = ('┌─┐┌─┐┌─┐┬┌─  ┬ ┬', '├─┘├─┤│  ├┴┐  └─┤', '┴  ┴ ┴└─┘┴ ┴    ┴')
CAPTION = ('Shandalar · gameplay pack 4', 'Homelands — local construction')
HINT = (
    'python3 tools/pack_4_homelands.py fetch      # refresh the set data',
    'python3 tools/pack_4_homelands.py fetch-art  # fetch the set artwork',
    'python3 tools/pack_4_homelands.py            # build the local ZIP',
    'python3 tools/pack_4_homelands.py verify     # validate the ZIP',
)
...
def fetch():
    meta = fetch_cards.scryfall_get('https://api.scryfall.com/sets/hml')
    url = meta['search_uri']
    rows = []
    while url:
    ...
    if len(rows) != page['total_cards'] or any(row['set'] != 'hml' for row in rows):
        raise ValueError('incomplete or mixed Scryfall response')
    records = [fetch_cards.trim(row, 'hml') for row in rows]
    SOURCE.mkdir(parents=True, exist_ok=True)
    (SOURCE / 'cards.json').write_bytes(json_bytes(records))
    (SOURCE / 'set.json').write_bytes(json_bytes({
        'code': 'hml', 'name': meta['name'], 'released_at': meta['released_at'],
    ...
    print('Homelands: %d printings, %d unique names' %
...
def assembled():
    ...
    for row in rows:
        if row.get('set') != 'hml':
            raise ValueError('Pack 4 must contain only Homelands')
        by_name.setdefault(row['name'], row)
    cards = list(by_name.values())
    if len(rows) != 140 or len(cards) != 115:
        raise ValueError('Homelands checklist changed; review before building')
    # Checked against the original 897-card registry. This small trusted
    # snapshot also lets the distributed construction tool run without a
    # source checkout or any other optional pack.
    reprints = set(read_json(SOURCE / 'reprint_names.json'))
    if len(reprints) != 0 or not reprints <= set(by_name):
        raise ValueError('Homelands reprint checklist changed; review before building')
    ...
    catalog = {'pack_format': 1, 'pack_id': 'pack-4', 'counts': counts,
               'sets': {'hml': {'names': sorted(by_name), 'named_cards': len(cards),
...
def art_targets(art_dir, cards=None):
    ...
    return [(path, row, variant) for row in cards
            for path, variant in fetch_card_art.targets_for(row['name'], art_dir / 'hml')]

def fetch_art(art_dir):
    failed = 0
    cards = assembled()[2]
    for index, row in enumerate(cards, 1):
        targets = fetch_card_art.targets_for(row['name'], art_dir / 'hml')
        missing = [(path, variant) for path, variant in targets if not path.is_file()]
        if missing:
            for path, _ in missing:
                path.parent.mkdir(parents=True, exist_ok=True)
            _, bad = fetch_card_art.fetch_missing_art(row['name'], 'hml', missing)
            failed += bad
        if index % 10 == 0 or index == len(cards):
            print('Homelands artwork: %d/%d names' % (index, len(cards)), flush=True)
...
def build(out, art_dir=DEFAULT_ART, include_art=True):
    ...
    if include_art:
        for path, row, _ in art_targets(art_dir, cards):
            ...
            artwork.append((PREFIX + 'art/hml/' + path.name, payload))
    ...
    with tempfile.TemporaryDirectory(prefix='.pack-4-', dir=out.parent) as tmp:
    ...
    print('Pack 4: 115 unique cards · 140 printings -> %s' % out)
...
def verify(path, require_art=True):
    ...
    with zipfile.ZipFile(path) as zf:
        ...
        for target, row, _variant in art_targets(Path('unused'), cards):
            art_names.add(PREFIX + 'art/hml/' + target.name)
            if row['name'] in manifest['new_rules_identities']:
