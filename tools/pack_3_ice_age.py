#!/usr/bin/env python3
"""Build, fetch, or verify the separate Ice Age gameplay pack.
...
SOURCE = ROOT / 'packaging/card_packs/pack_3_ice_age'
PREFIX = 'card_packs/pack_3_ice_age/'
FILE_NAME = 'Pack-3-Ice_Age.zip'
DEFAULT_OUT = ROOT.parent / 'shandalar-packs' / FILE_NAME
DEFAULT_ART = ROOT.parent / 'shandalar-packs/cache/pack_3_art'
TOOL = 'pack_3_ice_age.py'
WORDMARK = ('┌─┐┌─┐┌─┐┬┌─  ┌─┐', '├─┘├─┤│  ├┴┐   ─┤', '┴  ┴ ┴└─┘┴ ┴  └─┘')
CAPTION = ('Shandalar · gameplay pack 3', 'Ice Age — local construction')
HINT = (
    'python3 tools/pack_3_ice_age.py fetch      # refresh the set data',
    'python3 tools/pack_3_ice_age.py fetch-art  # fetch the set artwork',
    'python3 tools/pack_3_ice_age.py            # build the local ZIP',
    'python3 tools/pack_3_ice_age.py verify     # validate the ZIP',
)
...
def fetch():
    meta = fetch_cards.scryfall_get('https://api.scryfall.com/sets/ice')
    url = meta['search_uri']
    rows = []
    while url:
    ...
    if len(rows) != page['total_cards'] or any(row['set'] != 'ice' for row in rows):
        raise ValueError('incomplete or mixed Scryfall response')
    records = [fetch_cards.trim(row, 'ice') for row in rows]
    SOURCE.mkdir(parents=True, exist_ok=True)
    (SOURCE / 'cards.json').write_bytes(json_bytes(records))
    (SOURCE / 'set.json').write_bytes(json_bytes({
        'code': 'ice', 'name': meta['name'], 'released_at': meta['released_at'],
    ...
    print('Ice Age: %d printings, %d unique names' %
...
def assembled():
    ...
    for row in rows:
        if row.get('set') != 'ice':
            raise ValueError('Pack 3 must contain only Ice Age')
        by_name.setdefault(row['name'], row)
    cards = list(by_name.values())
    if len(rows) != 383 or len(cards) != 373:
        raise ValueError('Ice Age checklist changed; review before building')
    ...
    if len(reprints) != 27 or not reprints <= set(by_name):
        raise ValueError('Ice Age reprint checklist changed; review before building')
    ...
    catalog = {'pack_format': 1, 'pack_id': 'pack-3', 'counts': counts,
               'sets': {'ice': {'names': sorted(by_name), 'named_cards': len(cards),
...
def art_targets(art_dir, cards=None):
    ...
    return [(path, row, variant) for row in cards
            for path, variant in fetch_card_art.targets_for(row['name'], art_dir / 'ice')]

def fetch_art(art_dir):
    failed = 0
    cards = assembled()[2]
    for index, row in enumerate(cards, 1):
        targets = fetch_card_art.targets_for(row['name'], art_dir / 'ice')
        missing = [(path, variant) for path, variant in targets if not path.is_file()]
        if missing:
            for path, _ in missing:
                path.parent.mkdir(parents=True, exist_ok=True)
            _, bad = fetch_card_art.fetch_missing_art(row['name'], 'ice', missing)
            failed += bad
        if index % 10 == 0 or index == len(cards):
            print('Ice Age artwork: %d/%d names' % (index, len(cards)), flush=True)
...
def build(out, art_dir=DEFAULT_ART, include_art=True):
    ...
    if include_art:
        for path, row, _ in art_targets(art_dir, cards):
            ...
            artwork.append((PREFIX + 'art/ice/' + path.name, payload))
    ...
    with tempfile.TemporaryDirectory(prefix='.pack-3-', dir=out.parent) as tmp:
    ...
    print('Pack 3: 373 unique cards · 383 printings -> %s' % out)
...
def verify(path, require_art=True):
    ...
    with zipfile.ZipFile(path) as zf:
        ...
        for target, row, _variant in art_targets(Path('unused'), cards):
            art_names.add(PREFIX + 'art/ice/' + target.name)
            if row['name'] in manifest['new_rules_identities']:
