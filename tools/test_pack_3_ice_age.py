"""Offline checks for the dedicated Ice Age construction tool."""
import tempfile
import subprocess
import unittest
import warnings
...
import pack_3_ice_age as pack
import pack_1_dotp_complete as first

class PackThreeTests(unittest.TestCase):
    def test_release_guard_refuses_each_numbered_pack_but_allows_source(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / 'pack_3_ice_age.py').write_text('# construction source\n')
            def check():
                return subprocess.run(['bash', '-c', guard + 'guard_stage "$1"', 'guard-test', str(root)],
                                      capture_output=True, text=True)
            self.assertEqual(check().returncode, 0)
            for filename in ['Pack-1-DotP-complete.zip', 'Pack-2-Fallen-Empires.zip', pack.FILE_NAME]:
    ...
    def test_full_checklist_and_independent_names(self):
        ...
        self.assertEqual(len(names), 373)
        self.assertEqual(len(pack.read_json(pack.SOURCE / 'cards.json')), 383)
        self.assertEqual(set(catalog['sets']), {'ice'})
        self.assertEqual(set(catalog['sets']['ice']['names']), names)
        self.assertEqual(names & {name for _, name in first.assigned_pairs()},
                         set(pack.read_json(pack.SOURCE / 'reprint_names.json')))
        self.assertEqual(len(manifest['new_rules_identities']), 346)
        self.assertEqual(len(set(manifest['new_rules_identities'])), 346)
        self.assertEqual(manifest['id'], 'pack-3')
        self.assertEqual(len(pack.art_targets(Path('unused'))), 746)
    ...
    def test_deterministic_art_archive_and_no_scripts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(pack.verify(out)['checksums']['artwork']['files'], 1438)
            with zipfile.ZipFile(out) as archive:
                self.assertEqual(len(archive.namelist()), 1442)
