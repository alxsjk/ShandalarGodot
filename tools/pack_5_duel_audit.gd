extends SceneTree
## Deterministic complete Wizard-v-Wizard Alliances duels in both rulesets.
...
func _run() -> void:
	...
	if args.has("--version") or args.has("-V"):
		print("pack_5_duel_audit.gd ", ProjectSettings.get_setting("application/config/version", "unknown"))
		quit(0)
		return
	if args.has("--help") or args.has("-h"):
		print("Pack 5 Alliances audit — complete Wizard duels and actual card-use counts in both rulesets.")
		print("--rounds N (default 1, max 1000); --seed N (default 57000). Nine themed decks, 18 duels per round.")
		...
		print("Set SHANDALAR_PACK_5 to your locally built Pack-5-Alliances.zip; run with --script res://tools/pack_5_duel_audit.gd.")
		quit(0)
		return
	var rounds := 1
	var base_seed := 57000
	...
	if not OS.has_feature("shandalar_test"):
		printerr("PACK 5 AI AUDIT: requires GODOT_EDITOR_CUSTOM_FEATURES=shandalar_test")
		quit(2)
		return
	Settings.set_value("enabled_card_packs", ["pack-5"], false)
	root.get_node("CardPacks")._configure_registry()
	CardRegistry.ensure_loaded()
	if CardRegistry.size() != 1041:
		printerr("PACK 5 AI AUDIT: install the real local Pack-5-Alliances.zip first")
		quit(2)
		return
	var specs := [
		[
	...
	for spec in specs:
		...
		for name in spec[1]:
			if not CardRegistry.has_card(name):
				printerr("PACK 5 AI AUDIT: missing " + name)
				quit(2)
				return
			for _i in 4: deck.append(name)
		for name in ["Phyrexian War Beast", "Soldevi Digger", "Helm of Obedience", "Astrolabe"]: deck.append(name)
		decks.append(deck)
	var completed := 0
	var use_counts := {}
	for edition in ["modern", "fifth"]:
		for match_index in decks.size() * rounds:
			...
			if not AiPlayer.play_out(game, a, b):
				printerr("PACK 5 AI AUDIT FAILED: ", edition, " seed ", duel_seed, " turn ", game.turn_number)
			...
			print("PACK 5 AI DUEL OK: ", edition, " seed ", duel_seed, " turns ", game.turn_number, " winner ", game.winner)
	var keys: Array = use_counts.keys()
	keys.sort()
	for key in keys: print("PACK 5 ACTUAL USE: ", key, " = ", use_counts[key])
	print("PACK 5 AI AUDIT OK: ", completed, " completed full duels")
	quit(0)
