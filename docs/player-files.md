...
| **The card folder** | `user://cardpacks/*.zip` | Ordinary art ZIPs contain `skin/cardart/<snake_name>.jpg\|png`; they are mounted at boot in name order, the first to hold a picture winning. `Pack-1-DotP-complete.zip` is the one exact gameplay-pack exception: `CardPacks` validates its version, minimum game version, metadata/art checksums, 373 set entries and art list before showing the compact `1-tDotP` title button. **Options → Card Packs** shows its path, version, enabled state and any rejection reason, with Open Folder and Rescan controls. Disabled leaves the default 897-card pool; enabled reports `1,270 set entries · 901 unique cards`. Saved decks declare Pack 1 when they use one of its four new identities, offer to enable it on load, and remain card-name-based rather than silently pinning a reprint. The ZIP is built locally with `tools/pack_1_dotp_complete.py` and is never a release payload. `cardpacks_folder` may move the folder; browser **Forget my zips** clears only the managed `user://` folder. |
...
| The card scripts | `res://cards/sets/<set>/*.gd` — one file per default card, 897 of them; `res://cards/optional/pack_1/*.gd` — four trusted dormant implementations, never scripts loaded from a ZIP |
