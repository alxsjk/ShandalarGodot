...
|---|---|---|---|
| Bounty of the Hunt; Thawing Glaciers (Pack 5) | Counters are removed / the land returns at the correct cleanup boundary, but these cleanup actions do not open a stack-response window. They do not happen prematurely in the end step. | Repeatable cleanup priority and delayed-trigger handling under CR 514.3a. | Either player loses the chance to respond to these delayed cleanup triggers; ordinary end-step responses remain available. |
