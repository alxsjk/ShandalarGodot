class_name FallenEmpiresPack
extends RefCounted
## Trusted Pack 2 contract. The ZIP supplies data/art, never script paths.

const ID := "pack-2"
const FILE_NAME := "Pack-2-Fallen-Empires.zip"
const PREFIX := "card_packs/pack_2_fallen_empires/"
const SOURCE := "res://packaging/card_packs/pack_2_fallen_empires/"
const COUNTS := {"published_printings": 187, "named_set_entries": 102,
	"distinct_cards": 102, "pack_card_entries": 102,
	"reprint_entries": 0, "new_rules_identities": 102}
...
static func scripts() -> Array:
	var out: Array = []
	for name in names():
		out.append({"name": name, "set": "fem",
			"path": "res://cards/sets/fem/%s.gd" % snake(name)})
	return out
...
static func _inspect(reader: ZIPReader) -> Dictionary:
	...
	for name in names():
		for suffix in [".jpg", "_card.jpg"]:
			artwork.append(PREFIX + "art/fem/" + snake(name) + suffix)
	...
	if not has_art and (got != core or not OS.has_feature("shandalar_test")):
		return {"ok": false, "why": "expected the exact metadata and all 408 Pack 2 artwork files; no scripts are allowed"}
	...
	if not (manifest is Dictionary and catalog is Dictionary and cards is Array):
		return {"ok": false, "why": "invalid Pack 2 JSON"}
	...
	for key in trusted:
		if manifest.get(key) != trusted[key]:
			return {"ok": false, "why": "Pack 2 manifest mismatch: " + String(key)}
	...
	if not _same(manifest.get("counts"), COUNTS) or not _same(manifest.get("new_rules_identities"), names()):
		return {"ok": false, "why": "Pack 2 counts or names do not match Fallen Empires"}
	var wanted_catalog := {"pack_format": 1, "pack_id": ID, "counts": COUNTS,
		"sets": {"fem": {"names": names(), "named_cards": 102, "published_printings": 187}}}
	if not _same(catalog, wanted_catalog) or cards != records():
		return {"ok": false, "why": "Pack 2 checklist differs from the trusted Fallen Empires data"}
	var sums := {}
