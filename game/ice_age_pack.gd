class_name IceAgePack
extends RefCounted
## Trusted Pack 3 contract. The ZIP supplies data/art, never script paths.

const ID := "pack-3"
const FILE_NAME := "Pack-3-Ice_Age.zip"
const PREFIX := "card_packs/pack_3_ice_age/"
const SOURCE := "res://packaging/card_packs/pack_3_ice_age/"
const COUNTS := {"published_printings": 383, "named_set_entries": 373,
	"distinct_cards": 373, "pack_card_entries": 373,
	"reprint_entries": 27, "new_rules_identities": 346}
...
static func scripts() -> Array:
	...
	for name in new_names():
		out.append({"name": name, "set": "ice",
			"path": "res://cards/sets/ice/%s.gd" % snake(name)})
	return out
...
static func _inspect(reader: ZIPReader) -> Dictionary:
	...
	for name in names():
		for suffix in [".jpg", "_card.jpg"]:
			artwork.append(PREFIX + "art/ice/" + snake(name) + suffix)
	...
	if not has_art and (got != core or not OS.has_feature("shandalar_test")):
		return {"ok": false, "why": "expected the exact metadata and all 1438 Pack 3 artwork files; no scripts are allowed"}
	...
	if not (manifest is Dictionary and catalog is Dictionary and cards is Array):
		return {"ok": false, "why": "invalid Pack 3 JSON"}
	...
	for key in trusted:
		if manifest.get(key) != trusted[key]:
			return {"ok": false, "why": "Pack 3 manifest mismatch: " + String(key)}
	...
	if not _same(manifest.get("counts"), COUNTS) or not _same(manifest.get("new_rules_identities"), new_names()):
		return {"ok": false, "why": "Pack 3 counts or names do not match Ice Age"}
	var wanted_catalog := {"pack_format": 1, "pack_id": ID, "counts": COUNTS,
		"sets": {"ice": {"names": names(), "named_cards": 373, "published_printings": 383}}}
	if not _same(catalog, wanted_catalog) or cards != records():
		return {"ok": false, "why": "Pack 3 checklist differs from the trusted Ice Age data"}
	var sums := {}
	...
	if not _same(manifest.get("checksums"), expected):
		return {"ok": false, "why": "Pack 3 metadata or artwork checksum does not match"}
